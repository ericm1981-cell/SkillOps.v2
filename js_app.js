...
const App = (() => {
  ...
  async function init() {
    try {
      ...
      // Expose for pages.js — authority device always has full setup access
      ...
      if (AppState.users.length === 0) {
        Boot.step('First run — creating admin user…');
        await lineService.addUser('Eric Martin', 'admin', []);
        await lineService.loadUsers();
        Boot.step('Admin user created ✓');
      }
        ...
        <header class="topbar" id="topbar">
          ...
          <div class="topbar-actions">
            <button class="btn btn-ghost btn-sm" id="session-btn" style="font-size:11px;font-family:monospace" title="Switch user">
              👤 <span id="session-btn-label">${t('session_who')}</span>
            </button>
    ...
    Modal.open({
      ...
      body: users.length === 0
        ? `<div class="empty-state" style="padding:var(--sp-6)">
             ...
             <span style="font-size:11px;color:var(--text-3)">Go to Setup → Users to add supervisors and team leads.</span>
           </div>`
        : `<div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--sp-3)" id="session-picker-grid">
            ${users.map(u => `
              <button class="btn ${current && current.id === u.id ? 'btn-primary' : 'btn-secondary'}"
                ...
                <span>${u.name}</span>
                <span style="font-size:9px;opacity:.7">${u.role === 'admin' ? '★ Admin' : u.role === 'teamlead' ? 'Team Lead' : 'Supervisor'}</span>
              </button>`).join('')}
          </div>
