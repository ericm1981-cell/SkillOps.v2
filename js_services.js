...
const skillService = {
  ...
  async setAgreed(lineId, employeeId, positionId, agreed) {
    const now = Date.now();
    // Try find existing open rec for this emp+pos
    const existing = await DB.getByIndex('pendingRecommendations', 'empPos', IDBKeyRange.only([employeeId, positionId])).catch(()=>null);
    if (existing && existing.status === 'open') {
      await DB.put('pendingRecommendations', { ...existing, agreed: !!agreed, updatedAt: now });
      return existing.id;
    }
    ...
    const rec = {
      ...
      createdAt: now,
      updatedAt: now,
      clientId: existing?.clientId || _uuid(),
      syncedToAuthority: false
    };

    // If there is an old actioned record, keep it and create a new open record.
    try {
      return await DB.add('pendingRecommendations', rec);
    } catch (e) {
      // Fallback: if unique constraint blocks, upsert with PUT using id if present
      if (existing && existing.id) {
        await DB.put('pendingRecommendations', { ...existing, ...rec, id: existing.id });
        return existing.id;
      }
      throw e;
    }
...
const rotationService = {
  async generate(lineId, date, bbMode) {
    ...
    const plan = {
      ...
      // Store present operators so the UI can show "Unassigned" per period
      presentEmployeeIds: presentEmployees.map(e => e.id),
...
const importExportService = {
  ...
  async exportExcel(lineId) {
    ...
    const wb = XLSX.read(tmRecord.templateBinary, { type: 'array', cellStyles: true, sheetStubs: true });
    const ws = wb.Sheets[tmRecord.sheetName];
    const m = tmRecord;

    // Preserve / enforce page setup to match the template (critical for print-ready output).
    // NOTE: SheetJS CE does not always round-trip page setup; we set common fields explicitly.
    ws['!pageSetup'] = ws['!pageSetup'] || {};
    if (!ws['!pageSetup'].orientation) ws['!pageSetup'].orientation = 'landscape';
    if (!ws['!pageSetup'].paperSize) ws['!pageSetup'].paperSize = 17; // matches Viwinco template (e.g., Tabloid 11x17)
    if (ws['!pageSetup'].fitToHeight == null) ws['!pageSetup'].fitToHeight = 0;
    if (ws['!pageSetup'].fitToWidth == null) ws['!pageSetup'].fitToWidth = 1;
    if (!ws['!pageSetup'].scale) ws['!pageSetup'].scale = 81;


    // Style references (template often stores styles only on some cells).
    const headerStyleRef = ws[XLSX.utils.encode_cell({ r: m.headerRow, c: m.firstPositionCol })];
    const rosterStyleRef = ws[XLSX.utils.encode_cell({ r: m.firstRosterRow, c: m.rosterCol })];
    const gridStyleRef   = ws[XLSX.utils.encode_cell({ r: m.firstRosterRow, c: m.firstPositionCol })];
    ...
    sortedPos.forEach((pos, i) => {
      ...
      else ws[addr] = { t: 's', v: pos.name, s: headerStyleRef?.s };
    });
    ...
    activeEmps.forEach((emp, ei) => {
      ...
      else ws[rosterAddr] = { t: 's', v: emp.name, s: rosterStyleRef?.s };
      ...
      sortedPos.forEach((pos, pi) => {
        ...
        else ws[cellAddr] = { t: 'n', v: level, s: gridStyleRef?.s };
      });
    });
    const date = new Date().toISOString().slice(0, 10);
    const line = AppState.lines.find(l => l.id === lineId);
    XLSX.writeFile(wb, `SkillMatrix_${line ? line.name.replace(/\s/g, '_') : lineId}_${date}.xlsx`, { bookType: 'xlsx', cellStyles: true });
  }
};
