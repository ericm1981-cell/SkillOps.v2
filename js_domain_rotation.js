  ...
  function generate({ presentEmployees, positions, qualifiedSet, l2Set, yesterdayPlan, bbMode }) {
    ...
    periods.forEach(period => {
      ...
      positions.forEach(pos => {
        ...
        // Pool4: L2 not in period (fallback when no L3/L4 available)
        const pool4 = presentEmployees.filter(e =>
          l2Set.has(key(e.id)) && !periodUsed.has(e.id)
        );

        // Pool5: L2 already in period (last resort)
        const pool5 = presentEmployees.filter(e =>
          l2Set.has(key(e.id)) && periodUsed.has(e.id)
        );
        ...
        if (pool1.length) {
          ...
          // No L3/L4 available — use L2 as planned fallback (not a violation, just underqualified)
          chosen = _tieBreak(pool4, todayCount, yesterdayMap, pos.id);
          violationReason = 'violation_underqualified';
        } else if (pool5.length) {
          chosen = _tieBreak(pool5, todayCount, yesterdayMap, pos.id);
          violationReason = 'violation_underqualified';
        }
