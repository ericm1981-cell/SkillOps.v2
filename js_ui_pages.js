...
Pages.matrix = function() {
  ...
  document.getElementById('export-snapshot-btn')?.addEventListener('click', async () => {
    try {
      await importExportService.exportLineSnapshot(lineId);
      document.getElementById('snapshot-status').innerHTML = `<div class="info-box success">Snapshot exported.</div>`;
    } catch(e) {
      document.getElementById('snapshot-status').innerHTML = `<div class="info-box error">${safe(e.message)}</div>`;
    }
  });
  ...
  document.getElementById('import-snapshot-file')?.addEventListener('change', async (e) => {
    ...
    try {
      ...
      await importExportService.importLineSnapshot(snapshot);
      document.getElementById('snapshot-status').innerHTML = `<div class="info-box success">Snapshot imported.</div>`;
    } catch(err) {
      document.getElementById('snapshot-status').innerHTML = `<div class="info-box error">${safe(err.message || String(err))}</div>`;
    } finally {
      e.target.value = '';
    }
  });
  ...
  _rc(`
    ...
    <div id="tab-export" class="tab-panel">
      ...
      <div class="card" style="margin-top:var(--sp-4)">
        <div class="card-header"><span class="card-title">Local backup (JSON snapshot)</span></div>
        <div class="card-body">
          <p style="color:var(--text-2);margin-bottom:var(--sp-3)">Export/import a full line snapshot to keep data on your device (employees, positions, skill levels, rotation plans, logs, and the saved Excel template mapping).</p>
          <div style="display:flex;flex-wrap:wrap;gap:var(--sp-3);align-items:center">
            <button class="btn" id="export-snapshot-btn">Export snapshot (.json)</button>
            <label class="btn" style="cursor:pointer">
              Import snapshot…
              <input type="file" id="import-snapshot-file" accept=".json" style="display:none">
            </label>
          </div>
          <div id="snapshot-status" style="margin-top:var(--sp-3)"></div>
        </div>
      </div>
